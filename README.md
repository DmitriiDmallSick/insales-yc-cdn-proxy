...
Проект проксирует ресурсы `static.insales-cdn.com` через Yandex Cloud CDN
и переписывает CDN-ссылки, которые InSales динамически добавляет в DOM.

Подходит, если:
- ресурсы InSales CDN недоступны части пользователей;
- у вас есть доступ к шаблонам темы;
- вы готовы протестировать корзину и виджеты после внедрения.

Не подходит как универсальное решение без адаптации под конкретную тему.
